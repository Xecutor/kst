...
#include <kst/ExplicitIntCast.hpp>
...
public:
  ...
  void Swap(File& swp) {
    ...
    if (flags & FLG_WRBUF)
      Flush();
    if (swp.flags & FLG_RDBUF)
      swp.flags &= ~FLG_RDBUF;
    if (swp.flags & FLG_WRBUF)
      swp.Flush();
  ...
  void ROpen(const char* fn) {
    ...
    if (fd == -1)
      throw FileException(FileException::errOpenFailed, fn);
  }
  ...
  size_t Read(void* buf, size_t sz) {
    ...
    if ((flags & FLG_BUFFERED) && (flags & FLG_RDBUF) != FLG_RDBUF && sz < bufferSize) {
      bufferUsed = read(fd, buffer, ExplicitIntCast(bufferSize));
      bufferPosition = 0;
      flags |= FLG_RDBUF;
    }
    ...
    size_t rv = read(fd, buf, ExplicitIntCast(sz));
    if (rv != sz) {
      if (errno)
  ...
  offset_type XRead(T& t) {
    return static_cast<offset_type>(Read(&t, sizeof(T)));
  }
  void Write(const void* buf, size_t sz) {
    Check();
    if (flags & FLG_RDBUF) {
      long off = ExplicitIntCast(bufferUsed - bufferPosition);
    ...
    if (flags & FLG_BUFFERED) {
      ...
      if (bufferPosition + sz > bufferSize) {
        ...
        sz -= towr;
        bufferPosition = 0;
        if (write(fd, buffer, ExplicitIntCast(bufferSize)) != (ssize_t)bufferSize) {
          throw FileException(FileException::errWriteFailed, filename.c_str());
        }
        if (sz > bufferSize) {
          if (write(fd, buf, ExplicitIntCast(sz)) != (ssize_t)sz) {
            throw FileException(FileException::errWriteFailed, filename.c_str());
          }
        } else {
  ...
  void Seek(offset_type off, int whence = SEEK_SET) {
    ...
    if (flags & FLG_RDBUF) {
      if (whence == SEEK_CUR) {
        ...
        off -= static_cast<offset_type>(bufferUsed - bufferPosition);
      }
      flags &= ~FLG_RDBUF;
    }
    if (lseek(fd, off, whence) == -1)
      throw FileException(FileException::errSeekFailed, filename.c_str());
  }
  ...
  offset_type Pos() {
    ...
    if (flags & FLG_RDBUF) {
      return static_cast<offset_type>(realPos - bufferUsed + bufferPosition);
    }
    if (flags & FLG_WRBUF) {
      return static_cast<offset_type>(realPos + bufferPosition);
    }
    return realPos;
  }
  ...
  void Flush() {
    ...
    if (flags & FLG_WRBUF) {
      if (bufferPosition > 0) {
        if (write(fd, buffer, ExplicitIntCast(bufferPosition)) != (ssize_t)bufferPosition)
          throw FileException(FileException::errWriteFailed, filename.c_str());
        bufferPosition = 0;
      }
      flags &= ~FLG_WRBUF;
  ...
  bool ReadLine(std::string& str) {
    ...
    offset_type sz = Size();
    while (pos < sz) {
      pos += static_cast<offset_type>(Read(&c, 1));
      if (c == 0x0a) {
        if (str.length() && str[str.length() - 1] == 0x0d)
  ...
  void Rename(const char* newname) {
    if (rename(filename.c_str(), newname) != 0)
  ...
  static void Rename(const char* oldname, const char* newname) {
    if (rename(oldname, newname) != 0)
      throw FileException(FileException::errRenameFailed, oldname);
  }

  static void Unlink(const char* fn) {
    if (unlink(fn) != 0)
      throw FileException(FileException::errUnlinkFailed, fn);
  }

protected:
  enum { INIT_BUFFER_SIZE = 4096 };
  enum { MAX_BUFFER_SIZE = 64 * 1024 * 1024 };
  enum { FLG_BUFFERED = 1, FLG_RDBUF = 2, FLG_WRBUF = 4, FLG_AUTOBUFFERED = 8 };
